---
template: project.jade
title: Generador de Valores
subtitle: Instalación interactiva | interactive installation
year: 2012
img: [
    https://farm9.staticflickr.com/8010/7253779782_30d2103c71_z.jpg,
    https://farm9.staticflickr.com/8024/7253760522_7021474246_z.jpg,
    https://farm8.staticflickr.com/7214/7253750268_bc468ea307_z.jpg,
    https://farm9.staticflickr.com/8158/7253761888_61efeb88d6_z.jpg,
    https://farm8.staticflickr.com/7223/7253774150_a2c0e2c9c0_z.jpg,
    https://farm8.staticflickr.com/7223/7253788768_90c5070aea_z.jpg
]
code: https://github.com/martinez-zea/generador_valores
images: https://secure.flickr.com/photos/nerd_bots/sets/72157629857536424/with/7253788768/
links: [
    {title: Bienal de la Habana, url: http://www.bienalhabana.cult.cu/?secc=artistas_amp&idArtista=118},
    {title: ArtForum, url: http://www.artforum.com/inprint/issue=201209&id=36142}
]
---

**Generador de Valores** es una máquina que crea valor a partir de la interacción de sus usuarios.

Cuando alguien acciona la palanca, esta máquina genera un título valor digital e imprime algunos caracteres del certificado del título valor, usando una máquina de escribir eléctrica modificada. La máquina debe ser operada hasta el final del certificado, después el visitante puede quedarse con el documento.

El precio de cada título es el resultado del valor monetario del proyecto dividido por el número de copias impresas. Con cada nueva copia el valor de todos los títulos cambia.  Cuando alguien usa la máquina, el valor del proyecto sube en un pequeño porcentaje. De la misma forma, si nadie usa la máquina, el proyecto sufre un pequeño porcentaje de depreciación que reduce el capital progresivamente a cero.

Cada título es un objeto única que tiene una parte digital y otra física.  La unicidad esta determinada por el uso de llaves criptográficas.  Para cada título hay una llave privada y una pública.  La llave privada es almacenada en la base de datos del sistema y la pública es impresa en un bloque de caracteres en el certificado impreso.  

----

**Value Generator** is a machine that creates aesthetic and economic value from the interaction of its users.

The system is composed of a wooden lever, a modified electric typewriter, a computer, a projector, and an Internet connection. When someone pushes the lever, this machine generates a digital share and prints a few characters of the share certificate. The machine has to be operated until the end of the certificate; then the visitor can keep it.

The price of the each share is the result of the project's value divided by the number of printed copies. With each new print the value of all shares changes. Likewise, when someone uses the machine, its value rises a small percentage. In the same way, the project suffers a small percentage of depreciation that reduces the invested capital as the time passes.

Each share is a unique object that has a digital and physical part. Its uniqueness is determined by the use of cryptographic keys. For each share there is a public and a private key. The private key is stored in the database, and the public key is printed in a block of characters in the printed certificate. The decorative patterns of each print are regenerated on each following print using cellular automata algorithms. With the use of those tools, the system allows the owner of a share to verify its authenticity.

The paper used to print the certificates has been made specifically for this work in order to reinforce the uniqueness of the print.  This strategy is directly based on the use of traditional graphic techniques for the fabrication of paper money.

This work states a paradox relationship between the piece's value, the visitor's  physical work, and its receptivity among the public. As long as the machine is used for a longer time, capital will rise but, at the same time, the price of each share will reduce. If no one interacts with the work, the value of all the generated shares will slowly decrease down to zero.  The project explores the idea that art products are economic objects tightly related to the social context of art, to the relative perceptions of value and the tendency of speculation common in the art market.
